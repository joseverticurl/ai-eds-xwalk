...
### Creating Static HTML for Testing (MANDATORY)

**Critical:** Before writing JavaScript and CSS, you MUST create static HTML files based on the actual HTML structure that AEM generates. This validates your assumptions and prevents structure mismatches.

**Why Static HTML is Essential:**
- Validates that field order in XWalk model matches actual AEM output
- Catches edge cases (empty fields, optional fields, missing cells)
- Enables local testing without AEM environment
- Documents the actual structure contract
- Prevents index-based access errors

**Steps to Create Static HTML:**

1. **Configure XWalk Model First:**
...
2. **Author Content in AEM:**
   ...
   - Test with various field combinations (empty, filled, optional)
...
4. **Create Static HTML Files in Specs Folder:**
   - **Location:** `specs/{guideName}/{version}/{featureName}/{specific-functionality-name}/static-html/`
   - **Naming:** `test-structure.html` (or descriptive names like `test-with-title.html`, `test-empty-fields.html`)
   - **Purpose:** Temporary files for local testing and structure validation
   - **Important:** These files are NOT part of the EDS project structure

5. **Document Structure Contract:**
   - Analyze the actual HTML to identify:
     - Which fields generate rows vs cells
     - Field order and indices
     - Empty field behavior (do they generate cells?)
     - Optional field behavior (parent title, etc.)
   - Document the actual structure contract in comments

**Example Static HTML File Structure:**

```
specs/eds-guide/EDS-2026.1.0/creating-eds-block/relatedcards/static-html/
├── test-structure.html          # Basic structure with all fields
├── test-with-title.html         # With section title
├── test-without-title.html      # Without section title (optional)
├── test-empty-fields.html       # With some empty fields
└── test-multiple-items.html     # Multiple child items
```

**Example Static HTML Content:**

```html
<!-- specs/.../static-html/test-structure.html -->
<!-- This is a TEMPORARY file for testing. Delete after implementation. -->
<!-- Actual HTML structure generated by AEM for relatedcards block -->

<div class="relatedcards">
  <!-- Section title row (if title field is provided) -->
  <div>
  ...
  <div>
    ...
    <div>Badge Text 1</div>
    <div>Card Title 1</div>
    <div><a href="/path/to/page1">Link URL 1</a></div>
    <div>_self</div>
  </div>
  
  <!-- Second card item row -->
  <div>
    <div>
      <picture>
    ...
    <div>Badge Text 2</div>
    <div>Card Title 2</div>
    <div><a href="/path/to/page2">Link URL 2</a></div>
    <div>_blank</div>
  </div>
</div>
```
...
**Using Static HTML for Testing:**

1. **Test JavaScript Extraction:**
   - Load static HTML in browser
   - Test `decorate()` function with actual structure
   - Verify index-based access works correctly
   - Test edge cases (empty fields, missing cells)

2. **Test CSS Styling:**
   - Apply CSS to transformed structure
   - Verify responsive breakpoints
   - Test visual appearance matches design

3. **Validate Structure Contract:**
   - Compare actual HTML with expected structure
   - Update structure contract documentation if needed
   - Fix any mismatches before final implementation

**After Implementation:**

- [ ] **MANDATORY:** Request user to delete all temporary static HTML files from `specs/.../static-html/` folder
- [ ] Static HTML files are for development/testing only
- [ ] They should NOT be committed to the repository
- [ ] They should NOT be part of the EDS project structure

**Important Notes:**
- Static HTML files are temporary development artifacts
- They help validate structure before coding
- They must be deleted after implementation is complete
- They are stored in `specs/` folder, not in the EDS project root
...
2. **Create Static HTML for Testing (MANDATORY):**
   - [ ] Configure XWalk model in AEM
   - [ ] Author content in AEM with various field combinations
   - [ ] Extract actual HTML structure from AEM
   - [ ] Create static HTML files in `specs/.../static-html/` folder
   - [ ] Document actual structure contract (field indices, empty field behavior, etc.)
   - [ ] Test JavaScript extraction logic with static HTML
   - [ ] Test CSS styling with static HTML

3. **Write JavaScript:**
   - [ ] Use static HTML to validate structure assumptions
   - [ ] Document structure contract in JSDoc (based on actual HTML)
   - [ ] Access elements by index based on actual field order (from static HTML)
   - [ ] Transform to final structure
   - [ ] Add CSS classes to transformed elements
   - [ ] Test with static HTML before AEM integration

4. **Write CSS:**
   - [ ] Target the transformed structure (after `decorate()` runs)
   - [ ] Use classes added by JavaScript
   - [ ] Test with static HTML to verify styling
   - [ ] Do NOT style the initial AEM-generated HTML structure

5. **Cleanup (MANDATORY):**
   - [ ] Request user to delete all temporary static HTML files
   - [ ] Static HTML files should NOT be committed to repository
...
**Critical: Validate Field Order with Static HTML**

**MANDATORY:** After configuring the XWalk model, you MUST create static HTML files to validate the actual field order. Do NOT assume the structure matches your model - verify it with actual AEM-generated HTML.
...
**Solution:** Always create static HTML from actual AEM output to verify the real structure contract.

**Reference:** See "Understanding the Structure Contract" section in Part 1 and "Creating Static HTML for Testing" in Pre-Implementation section for detailed explanation.
...
3. **Create Static HTML for Testing (MANDATORY)** → Extract actual HTML structure from AEM (Pre-Implementation)
   - Author content in AEM with various field combinations
   - Extract actual HTML structure from AEM (view source or DevTools)
   - Create static HTML files in `specs/.../static-html/` folder
   - Document actual structure contract (field indices, empty field behavior)
   - Test JavaScript extraction logic with static HTML
   - Test CSS styling with static HTML

4. **JavaScript** → Implement block logic (`<block-name>.js`) (Part 1)
   - Use static HTML to validate structure assumptions
   - Write `decorate()` function based on actual HTML structure
   - Test with static HTML before AEM integration

5. **CSS** → Style the block (`<block-name>.css`) (Part 1)
   - Test CSS with static HTML to verify styling
   - Ensure styles target transformed structure
...
7. **AEM Validation** → Test in AEM authoring interface (Part 3)
   - Test in AEM authoring interface
   - Verify content renders correctly

8. **Cleanup (MANDATORY)** → Delete temporary static HTML files
   - Request user to delete all static HTML files from `specs/.../static-html/` folder
   - Static HTML files should NOT be committed to repository
...
4. **Create Static HTML Files:**
   - [ ] Create folder: `specs/{guideName}/{version}/{featureName}/{specific-functionality-name}/static-html/`
   - [ ] Create `test-structure.html` with basic structure (all fields)
   - [ ] Create additional test files as needed:
     - [ ] `test-with-title.html` (if parent has optional title)
     - [ ] `test-without-title.html` (if parent title is optional)
     - [ ] `test-empty-fields.html` (with some empty fields)
     - [ ] `test-multiple-items.html` (multiple child items)
   - [ ] Add comments documenting structure contract in each file
...
6. **Test with Static HTML:**
   - [ ] Load static HTML in browser
   - [ ] Test JavaScript extraction logic
   - [ ] Verify index-based access works correctly
   - [ ] Test CSS styling
...
- [ ] **Use static HTML to validate structure assumptions before coding**
...
- [ ] Document **structure contract** (index = meaning) in JSDoc based on actual HTML structure
- [ ] Import shared components as needed
- [ ] Extract data using **index-based access only** (e.g. `block.children[0]`, `row.children[1]`)
   - [ ] Use field indices from actual HTML structure (not assumed indices)
...
- [ ] **Test with static HTML** to verify extraction logic works correctly
...
- [ ] **Test with static HTML** to verify styling works correctly
...
### Phase 6: Cleanup - Delete Temporary Static HTML Files (MANDATORY)

**Objective:** Remove temporary static HTML files used for testing. These files should NOT be part of the EDS project structure.

**Steps:**
- [ ] **Request user to delete all static HTML files** from `specs/{guideName}/{version}/{featureName}/{specific-functionality-name}/static-html/` folder
- [ ] Verify no static HTML files remain in the specs folder
- [ ] Confirm static HTML files are NOT committed to repository

**Important:**
- Static HTML files are temporary development artifacts
- They should be deleted after implementation is complete and validated
- They are NOT part of the EDS project structure
- They should NOT be committed to the repository
...
- **MANDATORY:** Create static HTML from actual AEM output to verify field order and indices
...
5. **Create static HTML for testing** - Extract actual HTML from AEM before coding (MANDATORY)
6. **Reference existing code** - Rather than creating from scratch
...
2. **XWalk configuration:** Add definitions/models/filters directly to root-level JSON files:
   ...
   - `component-filters.json` - Nesting rules
3. **Static HTML for testing (MANDATORY):** Create static HTML files from actual AEM output before coding:
   - Location: `specs/{guideName}/{version}/{featureName}/{specific-functionality-name}/static-html/`
   - Purpose: Validate structure contract and test extraction logic
   - **MANDATORY:** Delete all static HTML files after implementation is complete
...
6. **Testing:** Test with static HTML first, then manual testing in browser and AEM authoring interface
...
- **MANDATORY:** Create static HTML from actual AEM output to validate structure contract before coding.
- **MANDATORY:** Delete all temporary static HTML files after implementation is complete.
