...
When design specifies a responsive grid, apply shared grid classes in `decorate()` instead of hardcoding pixel widths per element. Keep grid behavior in `styles/styles.css` and only attach semantic utility classes in JS.

```javascript
/**
 * Example: apply shared grid utilities to first content row.
 * Assumes grid utilities are defined globally in styles/styles.css.
 */
export default function decorate(block) {
